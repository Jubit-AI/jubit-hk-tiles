...
## Deferred follow-ups
- ✅ **Stylized-vector-illustration preset — IMPLEMENTED** (`--vector` / `vector=true`): a flatter, more graphic variant of the soft look — `uCelBands 3`, hard palette flats (`uPaletteMix 1.0`), bold clean outline (`uEdge 1.7`), no paper grain. Reads as a clean vector isometric-city illustration. Best at **dense / closer framings** (small zoomed-out buildings over-merge to one flat tone — use a smaller `view_height`). Sample: `docs/gallery/mong-kok-vector.png`. Additive — the soft-paper look remains the default.
