...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bake Central HK isometric tiles")
    ...
    try:
        with sync_playwright() as pw:
            ...
            for idx, lat, lon in tiles:
                q = urlencode({
                    "export": "true", "lat": lat, "lon": lon,
                    "width": WIDTH, "height": HEIGHT,
                    "azimuth": AZ, "elevation": EL, "view_height": VIEW_HEIGHT_M,
                })
                ...
                try:
                    ...
                    loaded = False
                    print(f"   ⚠️  tile {idx:02d}: TILES_LOADED timeout, capturing anyway")
                out_path = OUT / f"tile_{idx:02d}.png"
                page.screenshot(path=str(out_path))
                size = out_path.stat().st_size
                status = "ok" if loaded else "timeout"
                print(f"   {'✅' if loaded else '⚠️ '} tile_{idx:02d}.png "
                      f"({lat:.4f},{lon:.4f}) {size//1024}KB [{status}]")
                saved += 1
                page.close()
                ctx.close()
            browser.close()
    ...
    print(f"\n=== baked {saved} tile(s) → {OUT} (failed={failed}) ===")
